...
class VolumeScannerTest(shared_test_lib.BaseTestCase):
    ...
    def testGetVolumeIdentifiers(self):
        ...
        identifiers = test_scanner._GetVolumeIdentifiers(volume_system, test_options)
        self.assertEqual(len(identifiers), 2)
    ...
    def testNormalizedVolumeIdentifiersMBR(self):
        ...
        # Test error conditions.
        with self.assertRaises(errors.ScannerError):
            test_scanner._NormalizedVolumeIdentifiers(volume_system, ["p3"], prefix="p")
    ...
    def testScanEncryptedVolume(self):
        ...
        # Test error conditions.
        with self.assertRaises(errors.ScannerError):
            test_scanner._ScanEncryptedVolume(scan_context, None, test_options)

        scan_node = source_scanner.SourceScanNode(None)
        with self.assertRaises(errors.ScannerError):
            test_scanner._ScanEncryptedVolume(scan_context, scan_node, test_options)
    ...
    def testScanEncryptedVolumeOnBDE(self):
        ...
        self.assertEqual(bde_scan_node.type_indicator, definitions.TYPE_INDICATOR_BDE)
        ...
        test_options.credentials = [("password", "bde-TEST")]
    ...
    def testScanVolumeOnLVM(self):
        ...
        base_path_specs = []
        test_scanner._ScanVolume(
            scan_context, lvm_scan_node, test_options, base_path_specs
        )
        self.assertEqual(len(base_path_specs), 1)
        ...
        base_path_specs = []
        test_scanner._ScanVolume(
            scan_context, lvm_scan_node.sub_nodes[0], test_options, base_path_specs
        )
        self.assertEqual(len(base_path_specs), 1)
    ...
    def testScanVolumeOnRAW(self):
        ...
        base_path_specs = []
        test_scanner._ScanVolume(scan_context, scan_node, test_options, base_path_specs)
        self.assertEqual(len(base_path_specs), 1)
    ...
    def testGetBasePathSpecsOnMBR(self):
        ...
        test_ext_path_spec1 = path_spec_factory.Factory.NewPathSpec(
            definitions.PREFERRED_EXT_BACK_END,
            location="/",
...
class WindowsVolumeScannerTest(shared_test_lib.BaseTestCase):
    ...
    def testOpenFile(self):
        ...
        file_object = test_scanner.OpenFile("C:\\Windows\\System32\\config\\syslog")
