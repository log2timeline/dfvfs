...
class FATPathSpecTest(test_lib.PathSpecTestCase):
    ...
    def testInitialize(self):
        ...
        path_spec = fat_path_spec.FATPathSpec(location="/test", parent=self._path_spec)

        self.assertIsNotNone(path_spec)

        path_spec = fat_path_spec.FATPathSpec(identifier=1, parent=self._path_spec)
    ...
    def testComparable(self):
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: FAT, location: /test", ""]
        )
        ...
        expected_comparable = "\n".join(["type: TEST", "type: FAT, identifier: 1", ""])
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: FAT, identifier: 1, location: /test", ""]
        )
