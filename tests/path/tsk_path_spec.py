...
class TSKPathSpecTest(test_lib.PathSpecTestCase):
    ...
    def testInitialize(self):
        ...
        path_spec = tsk_path_spec.TSKPathSpec(location="/test", parent=self._path_spec)
        ...
        path_spec = tsk_path_spec.TSKPathSpec(inode=0, parent=self._path_spec)
    ...
    def testComparable(self):
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: TSK, location: /test", ""]
        )
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: TSK, data stream: test, location: /test", ""]
        )

        self.assertEqual(path_spec.comparable, expected_comparable)

        path_spec = tsk_path_spec.TSKPathSpec(inode=1, parent=self._path_spec)

        self.assertIsNotNone(path_spec)

        expected_comparable = "\n".join(["type: TEST", "type: TSK, inode: 1", ""])
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: TSK, inode: 1, location: /test", ""]
        )
