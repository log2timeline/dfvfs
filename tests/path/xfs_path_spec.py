...
class XFSPathSpecTest(test_lib.PathSpecTestCase):
    ...
    def testInitialize(self):
        ...
        path_spec = xfs_path_spec.XFSPathSpec(location="/test", parent=self._path_spec)

        self.assertIsNotNone(path_spec)

        path_spec = xfs_path_spec.XFSPathSpec(inode=1, parent=self._path_spec)
    ...
    def testComparable(self):
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: XFS, location: /test", ""]
        )
        ...
        expected_comparable = "\n".join(["type: TEST", "type: XFS, inode: 1", ""])
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: XFS, inode: 1, location: /test", ""]
        )
