...
class NTFSPathSpecTest(test_lib.PathSpecTestCase):
    ...
    def testInitialize(self):
        ...
        path_spec = ntfs_path_spec.NTFSPathSpec(mft_entry=0, parent=self._path_spec)
    ...
    def testComparable(self):
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: NTFS, location: \\test", ""]
        )
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: NTFS, data stream: test, location: \\test", ""]
        )

        self.assertEqual(path_spec.comparable, expected_comparable)

        path_spec = ntfs_path_spec.NTFSPathSpec(mft_entry=1, parent=self._path_spec)

        self.assertIsNotNone(path_spec)

        expected_comparable = "\n".join(["type: TEST", "type: NTFS, MFT entry: 1", ""])
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: NTFS, MFT attribute: 3, MFT entry: 1", ""]
        )
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: NTFS, location: \\test, MFT entry: 1", ""]
        )
