...
class EXTPathSpecTest(test_lib.PathSpecTestCase):
    ...
    def testInitialize(self):
        ...
        path_spec = ext_path_spec.EXTPathSpec(location="/test", parent=self._path_spec)

        self.assertIsNotNone(path_spec)

        path_spec = ext_path_spec.EXTPathSpec(inode=1, parent=self._path_spec)
    ...
    def testComparable(self):
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: EXT, location: /test", ""]
        )
        ...
        expected_comparable = "\n".join(["type: TEST", "type: EXT, inode: 1", ""])
        ...
        expected_comparable = "\n".join(
            ["type: TEST", "type: EXT, inode: 1, location: /test", ""]
        )
