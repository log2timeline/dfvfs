...
class BDEFileWithKeyChainTest(test_lib.ImageFileTestCase):
  ...
  with credentials within the key chain."""
...
class BDEFileWithoutKeyChainTest(test_lib.ImageFileTestCase):
  """The unit test for the BitLocker Drive Encryption (BDE) file-like object
  with credentials embedded within the path spec."""
  ...
  def setUp(self):
    """Sets up the needed objects used throughout the test."""
    super(BDEFileWithoutKeyChainTest, self).setUp()
    test_file = os.path.join(u'test_data', u'bdetogo.raw')
    self._os_path_spec = os_path_spec.OSPathSpec(location=test_file)
    self._bde_path_spec = bde_path_spec.BDEPathSpec(
        credentials={u'password': self._BDE_PASSWORD},
