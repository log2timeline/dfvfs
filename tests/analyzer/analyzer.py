...
class AnalyzerTest(shared_test_lib.BaseTestCase):
    ...
    def testGetSignatureScanner(self):
        ...
        signature_scanner = analyzer.Analyzer._GetSignatureScanner(specification_store)
    ...
    def testHelperRegistration(self):
        ...
        self.assertEqual(len(analyzer.Analyzer._analyzer_helpers), number_of_helpers)
    ...
    def testGetCompressedStreamTypeIndicatorsBZIP2(self):
        ...
        type_indicators = analyzer.Analyzer.GetCompressedStreamTypeIndicators(path_spec)
        self.assertEqual(type_indicators, expected_type_indicators)
    ...
    def testGetFileSystemTypeIndicators(self):
        ...
        path_spec = vshadow_path_spec.VShadowPathSpec(store_index=1, parent=path_spec)
    ...
    def testGetStorageMediaImageTypeIndicatorsPHDI(self):
        """Tests the GetStorageMediaImageTypeIndicator function on a .vmdk file."""
        test_file = self._GetTestFilePath(["hfsplus.hdd", "DiskDescriptor.xml"])
        self._SkipIfPathNotExists(test_file)

        path_spec = os_path_spec.OSPathSpec(location=test_file)

        expected_type_indicators = [definitions.TYPE_INDICATOR_PHDI]
        type_indicators = analyzer.Analyzer.GetStorageMediaImageTypeIndicators(
            path_spec
        )
        self.assertEqual(type_indicators, expected_type_indicators)
