...
class AnalyzerTest(shared_test_lib.BaseTestCase):
  ...
  def testGetStorageMediaImageTypeIndicatorsAFF4(self):
    """Tests the GetStorageMediaImageTypeIndicator function on AFF4 files."""
    test_file = self._GetTestFilePath(['aff4', 'Base-Linear.aff4'])
    self._SkipIfPathNotExists(test_file)

    path_spec = os_path_spec.OSPathSpec(location=test_file)

    expected_type_indicators = [definitions.TYPE_INDICATOR_AFF4]
    type_indicators = analyzer.Analyzer.GetStorageMediaImageTypeIndicators(
        path_spec)
    self.assertEqual(type_indicators, expected_type_indicators)
