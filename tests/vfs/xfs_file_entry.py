...
class XFSFileEntryTest(shared_test_lib.BaseTestCase):
    ...
    def setUp(self):
        ...
        self._xfs_path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_XFS, location="/", parent=self._raw_path_spec
        )
    ...
    def testGetAttributes(self):
        ...
        path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_XFS,
            inode=self._INODE_A_FILE,
            location="/a_directory/a_file",
            parent=self._raw_path_spec,
        )
        file_entry = self._file_system.GetFileEntryByPathSpec(path_spec)
        self.assertIsNotNone(file_entry)
    ...
    def testSubFileEntries(self):
        ...
        expected_sub_file_entry_names = ["a_directory", "a_link", "passwords.txt"]
...
class XFSFileEntryTestWithBigtime(shared_test_lib.BaseTestCase):
    """Tests the XFS file entry."""

    # pylint: disable=protected-access

    _INODE_A_DIRECTORY = 16131
    _INODE_A_FILE = 16132
    _INODE_A_LINK = 16135
    _INODE_ANOTHER_FILE = 16134

    def setUp(self):
        """Sets up the needed objects used throughout the test."""
        self._resolver_context = context.Context()
        test_path = self._GetTestFilePath(["xfs-bigtime.raw"])
        self._SkipIfPathNotExists(test_path)
    ...
    def testGetStatAttribute(self):
        ...
        self.assertIsNone(stat_attribute.device_number)
        self.assertEqual(stat_attribute.group_identifier, 1000)
        self.assertEqual(stat_attribute.inode_number, 16134)
    ...
    def testGetExtents(self):
        ...
        self.assertEqual(extents[0].extent_type, definitions.EXTENT_TYPE_DATA)
        self.assertEqual(extents[0].offset, 8290304)
        self.assertEqual(extents[0].size, 4096)
