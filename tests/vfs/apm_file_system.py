...
class APMFileSystemTest(shared_test_lib.BaseTestCase):
    ...
    def testFileEntryExistsByPathSpec(self):
        ...
        path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_APM, entry_index=0, parent=self._modi_path_spec
        )
        self.assertTrue(file_system.FileEntryExistsByPathSpec(path_spec))

        path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_APM, location="/p1", parent=self._modi_path_spec
        )
        self.assertTrue(file_system.FileEntryExistsByPathSpec(path_spec))

        path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_APM, entry_index=9, parent=self._modi_path_spec
        )
        self.assertFalse(file_system.FileEntryExistsByPathSpec(path_spec))

        path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_APM, location="/p0", parent=self._modi_path_spec
        )
        self.assertFalse(file_system.FileEntryExistsByPathSpec(path_spec))

        path_spec = path_spec_factory.Factory.NewPathSpec(
            definitions.TYPE_INDICATOR_APM, location="/p9", parent=self._modi_path_spec
        )
        self.assertFalse(file_system.FileEntryExistsByPathSpec(path_spec))
