...
class TSKPartitionFileEntryTestAPM(shared_test_lib.BaseTestCase):
  ...
  def testGetTSKVsPart(self):
    """Tests the GetTSKVsPart function."""
    path_spec = path_spec_factory.Factory.NewPathSpec(
        definitions.TYPE_INDICATOR_TSK_PARTITION, part_index=0,
        parent=self._os_path_spec)
    file_entry = self._file_system.GetFileEntryByPathSpec(path_spec)
...
class TSKPartitionFileEntryTestBSDDisklabel(shared_test_lib.BaseTestCase):
  """TSK partition file entry tests on BSD disklabel."""
  ...
  def testSize(self):
    ...
    self.assertIsNotNone(file_entry)
    self.assertEqual(file_entry.size, 4186112)
