...
then
	...
	for FILE in `find setup.py config dfvfs tests -name \*.py`;
	do
		echo "Checking: ${FILE}";
...
then
	...
	if test -n "${TOXENV}";
	then
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd dfvfs && tox -e ${TOXENV}";

	elif test ${TRAVIS_PYTHON_VERSION} = "2.7";
	then
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd dfvfs && python2 run_tests.py";
	else
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd dfvfs && python3 run_tests.py";
	fi
