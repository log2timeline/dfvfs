...
class VolumeScannerMediator:
    ...
    def GetVolumeIdentifiers(self, volume_system, volume_identifiers):
        ...
        Returns:
          list[str]: selected volume identifiers including prefix or None.
        """
        if volume_system.TYPE_INDICATOR == (definitions.TYPE_INDICATOR_APFS_CONTAINER):
    ...
    def _ScanVolume(self, scan_context, scan_node, options, base_path_specs):
        ...
        if scan_node.IsVolumeSystemRoot():
            ...
            path_spec = path_spec_factory.Factory.NewPathSpec(
                definitions.PREFERRED_NTFS_BACK_END,
                location=location,
...
class WindowsVolumeScanner(VolumeScanner):
    ...
    def ScanForWindowsVolume(self, source_path, options=None):
        ...
        if (
            not base_path_specs
            or scan_context.source_type == definitions.SOURCE_TYPE_FILE
        ):
            return False
        ...
        self._path_resolver.SetEnvironmentVariable("WinDir", self._windows_directory)
