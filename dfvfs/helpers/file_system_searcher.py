...
class FindSpec:
    ...
    def __init__(
        ...
        location_arguments = [
            argument
            for argument in (location, location_glob, location_regex)
            if argument
        ]
        ...
        if (
            location_arguments
            and isinstance(location_arguments[0], str)
            and not location_separator
        ):
    ...
    def _CheckFileEntryType(self, file_entry):
        ...
        return (
            self._CheckIsDevice(file_entry)
            or self._CheckIsDirectory(file_entry)
            or self._CheckIsFile(file_entry)
            or self._CheckIsLink(file_entry)
            or self._CheckIsPipe(file_entry)
            or self._CheckIsSocket(file_entry)
        )
    ...
    def _CheckIsDevice(self, file_entry):
        ...
        """
        if (
            definitions.FILE_ENTRY_TYPE_BLOCK_DEVICE not in (self._file_entry_types)
            and definitions.FILE_ENTRY_TYPE_CHARACTER_DEVICE
            not in (self._file_entry_types)
            and definitions.FILE_ENTRY_TYPE_DEVICE not in self._file_entry_types
        ):
            return False
    ...
    def _CompareWithLocationSegment(self, location_segment, segment_index):
        ...
        """
        if (
            self._location_segments is None
            or segment_index < 0
            or segment_index > self._number_of_location_segments
        ):
            return False
        ...
        if self._is_regex:
            return bool(
                segment_name.match(location_segment)
    ...
    def AtLastLocationSegment(self, segment_index):
        ...
        """
        return bool(
            self._location_segments is not None
            and segment_index >= self._number_of_location_segments
        )
    ...
    def ComparePathSpecLocation(self, path_spec, file_system, mount_point=None):
        ...
        if (
            mount_point
            and mount_point.type_indicator == definitions.TYPE_INDICATOR_OS
            and path_spec.type_indicator == definitions.TYPE_INDICATOR_OS
        ):
...
class FileSystemSearcher:
    ...
    def _FindInFileEntry(self, file_entry, find_specs, segment_index):
        ...
        for find_spec in find_specs:
            ...
            is_last_location_segment = find_spec.IsLastLocationSegment(segment_index)
